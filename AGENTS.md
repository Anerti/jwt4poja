...
- **Out of scope / not implemented:** movies, projections, rooms, and `PATCH`/`DELETE /users/{userId}`. `SecurityConfig`: `/auth/**` + health endpoints `permitAll`, `GET /users` ADMIN-only, everything else authenticated (per-resource owner/role checks live in `ResourcesAccessRules`, not the filter chain).
