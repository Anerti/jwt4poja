...
- **Auth endpoints** (hand-written, `endpoint/rest/controller/AuthController.java`): `POST /auth/register` (202 + verification email), `POST /auth/login` (202 + login-verification link), `POST /auth/resend-link` (202, `email` query param), `GET /auth/verification/{token}` (200 JWT + user, 401 invalid token).
...
- **ITs:** `src/test/.../endpoint/rest/controller/auth/` — `RegisterIT`, `AuthVerificationIT`.
...
```bash
export JAVA_HOME=$HOME/.jdks/ms-21.0.12 && export PATH=$JAVA_HOME/bin:$PATH   # java is not on PATH
...
- Branches: `config` (current work branch), `preprod` (deployment branch — pushes trigger CD via the Poja API), `origin/preprod` is the remote default.
