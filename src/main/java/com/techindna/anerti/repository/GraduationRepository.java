...
@Repository
public interface GraduationRepository extends JpaRepository<GraduateOutput, UUID> {
...
WHERE si.class_id = :classId AND si.student_status = 'GRADUATED'
ORDER BY COALESCE(sa.overall_average, 0) DESC
""",
      nativeQuery = true)
  java.util.List<Object[]> findGraduatesByClassId(@Param("classId") UUID classId);
}
